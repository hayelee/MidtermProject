		    ...
		    <div id="board-search">
		        <div class="container">
		            <div class="search-window">
		                <form action="<%=request.getContextPath()%>/notice/search2.do">
		        ...
		        <div class="container">
		            <table class="board-table">
		                ...
		                <tr>
		                    ...
		                    <td id="td-title"><a href="<%=request.getContextPath() %>/notice/detail2.do?noticeId=<%=noticeList.get(i).getNoticeId() %>"><%=noticeList.get(i).getNoticeTitle() %></a></td>
		                    <td>관리자</td>
...
<c:set var="currentPage" value="${requestScope.cPage }"></c:set>
	...
	<%-- prev 버튼 시작--%>  
	  <c:choose>
			...
			<c:otherwise>
			    <li class="page-item">
			      <a class="page-link" href="<%=request.getContextPath()%>/notice/list2.do?pageNo=${cPage-1}" aria-label="Previous" tabindex="-1">
			        <span aria-hidden="true">&laquo;</span>
			      </a>
			    </li>
			</c:otherwise>		
	...
	<%-- prev 버튼 끝--%>  
	  ...
	  <c:forEach var="i" begin="${requestScope.sPage}" end="${requestScope.ePage}">
	  	<c:if test="${currentPage == i }">
		    <li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/notice/list2.do?pageNo=${i}">${i}</a></li>
	  	</c:if>
		<c:if test="${currentPage != i }">
		    <li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/notice/list2.do?pageNo=${i}">${i}</a></li>
	  ...
	  <%-- next 버튼 시작--%>    
	      <c:choose>
			...
			<c:otherwise>
			     <li class="page-item">
			      <a class="page-link" href="<%=request.getContextPath()%>/notice/list2.do?pageNo=${cPage+1}" aria-label="Next">
			        <span aria-hidden="true">&raquo;</span>
			      </a>
			    </li>
			</c:otherwise>		
