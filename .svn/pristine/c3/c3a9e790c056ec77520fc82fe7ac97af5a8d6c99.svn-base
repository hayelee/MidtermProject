...
public class AtchFileServiceImpl implements IAtchFileService {
	...
	public AtchFileVO saveAtchFileList(HttpServletRequest req) {
		...
		// 업로드 경로 설정하기
		String uploadPath = "D:/A_TeachingMaterial/03_HighJava/workspace/DaangnProject/WebContent/" + UPLOAD_DIR;
		...
		try {
			...
			for (Part part : req.getParts()) {
				...
				if (fileName != null && !fileName.equals("")) {
					...
					// UUID -> 유니크한 id를 랜덤하게 추출해준다. '-' 이런게 붙는데 이게 보기 싫어서 걍 제거함
	...
	private String getFileName(Part part) {
		 ...
		 * filename="a.jpg" <- file인 경우 이게 날라온다.
		 * 
		 */
